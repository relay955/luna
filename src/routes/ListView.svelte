  ...
  <table>
    ...
    {#each fileItems as fileItem}
      <tr on:dblclick={()=>onDoubleClickFileItem(fileItem)}
          on:mousedown={(e)=>onMouseDownFileItem(e,fileItem)}>
        <td class="td-name">
          <div
              style={`display: flex; align-items: center; color:${fileItem.decrypted_name ? "#FFB84D":"#000000"}`}>
            {#if fileItem.decrypted_name}
              <div style=" width: 15px; height: 15px; margin-top: 2px; margin-left: 2px; margin-right: 7px;">
                <FaUnlock style="margin-right:5px;" />
              </div>
            {/if}
            {#if !fileItem.decrypted_name}
              <img src={`data:image/png;base64,${getIconBinary(iconCache,fileItem)}`}
                   alt="file_icon" width="20px" height="20px" style="margin-right:5px;"/>
            {/if}
            {fileItem.decrypted_name ?? fileItem.name}
